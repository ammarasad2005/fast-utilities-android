...
| **Class Change Alerts** | Opt-in **local** notifications for your **tagged** timetable the moment `timetable.json` changes: *cancelled · restored · rescheduled (day → day) · time changed · venue changed · added · removed*, merged into one summary when several land together. Powered by a pure-JS diff engine canonicalized against JSON-rewrite noise, the existing 15-min background sync (works with the app closed), and a tiny native `Notifier` Kotlin module — **zero new dependencies, no FCM/server**. First run seeds silently (no blast); de-duped across background + foreground checks. Enable it from the switch on the Timetable tab (Android 13+ asks for permission once). |
...
**Background sync:** an `expo-background-task` (Android WorkManager) also refreshes campus data into AsyncStorage periodically *while the app is closed* — battery-friendly and OS-scheduled (min ~15 min, deferred by Doze), so data stays eventually-fresh without the app running.

**Class change alerts (local, opt-in):** after each background sync (and on a throttled Timetable-tab focus), the app diffs your **tagged** timetable entries against the last-seen snapshot in AsyncStorage and posts a **local Android notification** for anything new — class cancelled, restored, rescheduled across days/slots, venue moved, sessions added/removed. The notifier is a tiny Kotlin module in `modules/widget-store` (channel `class_changes`, Android-13 `POST_NOTIFICATIONS` prompt via an opt-in switch on the Timetable tab) — no expo-notifications, no FCM, no extra dependencies or accounts. First run seeds a silent baseline (no "everything is new" blast); a 7-day signature ledger de-dupes across the background task and foreground checks.
